...
#define _GHashMapEntery_FUNC_NAME(key_t, value_t, rest)                        \
  _ghash_map_entery_##key_t##_##value_t##_##rest
...
#define DEF_GHASH_MAP_ENTERY(key_t, value_t, key_cmp_f, value_cmp_f)           \
  GHashMapEntery(key_t, value_t) {                                             \
    ...
    value_t value;                                                              \
                                                                               ...
                                                                               \
  bool _GHashMapEntery_FUNC_NAME(key_t, value_t, find_by_key) (                     \
      void *arg, GHashMapEntery(key_t, value_t) * it) {                        \
    return key_cmp_f(arg, (it->key));                                              \
  }                                                                            \
                                                                               \
  bool _GHashMapEntery_FUNC_NAME(key_t, value_t, find_by_value)(                   \
      void *arg, GHashMapEntery(key_t, value_t) * it) {                        \
...
#define DEF_GENERIC_HASH_MAP(key_t, value_t, nbuckets)                         \
  ...
  bool GHASH_MAP_FUNC_NAME(key_t, value_t, nbuckets, contains)(                \
      GHashMap(key_t, value_t, nbuckets) * ghm, key_t * key) {                 \
    size_t bucket = ghm->hash_f(key) % nbuckets;                               \
    return vec_##_GHashMapEntery_NAME(key_t, value_t, find)(                  \
               ghm->buckets + bucket,                                          \
  ...
  value_t *GHASH_MAP_FUNC_NAME(key_t, value_t, nbuckets, get)(                 \
      GHashMap(key_t, value_t, nbuckets) * ghm, key_t * key) {                 \
    size_t bucket = ghm->hash_f(key) % nbuckets;                               \
    return vec_##_GHashMapEntery_FUNC_NAME(key_t, value_t, find)(                  \
        ghm->buckets + bucket,                                                 \
